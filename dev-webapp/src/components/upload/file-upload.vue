...
export default {
  ...
  methods: {
    ...
    addInputFileWithPreload(el) {
      ...
      if (el.files) {
        // 上传数量限制
        let applyLength = this.maxLength > 0 ? this.maxLength - this.files.length : el.files.length
        let newLength = applyLength > el.files.length ? el.files.length : applyLength
        ...
        for (let i = 0; i < newLength; i++) {
          ...
          reader.onload = function (e) {
            ...
            image.onload = function () {
              ...
              count++
              if (count >= newLength) return _this.add(files)
            }
            image.src = data
          }
    ...
    change(e) {
      this.preload ? this.addInputFileWithPreload(e.target) : this.addInputFile(e.target)
    },
