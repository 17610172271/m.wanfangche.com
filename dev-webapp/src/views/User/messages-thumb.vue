<template>
  <div class="thumb-container">
    <div class="message-header">
      <navheader>
        <span @click="selectShow=!selectShow">{{$route.query.type==1 ? '收到的赞' : '发出的赞'}}</span>
    ...
    <scroll v-model="list" ref="scrollComponent">
      <div class="message-content">
        <div class="no-comment-list text-md text-muted" v-if="list.length == 0">暂时没有赞</div>
...
export default {
  ...
  methods: {
    ...
    typeSwitch (type) {
      this.$router.replace({name: 'messages_thumb', query: {type: type}})
    },
    onRouterChange () {
      let container1 = document.getElementsByClassName('app-container')[0]
      if (container1) {
